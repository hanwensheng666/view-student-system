...
import { editAct } from '@/api/activity'
class ModifyAct extends Component {
  ...
  onChange({ file, fileList, event }) {
    ...
    if (file.status === 'done') {
      ...
      let url = 'http://han.wensheng.xyz//' + file.response.key
  ...
  async deleteResource(file) {
    let imgUrl = ''
    if (file.url) {
      imgUrl = file.url.replace(/http:\/\/han.wensheng.xyz\//, '')
    ...
    if (this.props.location.state && this.props.location.state.stu) {
      ...
      let fileList = stu.banner.map((item, index) => {
        return {
          ...
          url:
            item.imgUrl +
            '?imageMogr2/crop/700x' +
            (window.is_support_webp ? '/format/webp' : ''),
          thumbUrl:
            item.imgUrl +
            '?imageMogr2/crop/700x' +
            (window.is_support_webp ? '/format/webp' : '')
  ...
  loadData = async selectedOptions => {
    ...
    let classList = await this.getAllClassByGrade(
  ...
  render() {
    return (
      <div className="modify-act">
        <Row>
          <Col span={9} offset={7}>
            <h3 className="modify-act__title">编辑活动信息</h3>
          </Col>
        </Row>
        <Row>
          <Col span={9} offset={7}>
            <Form onSubmit={this.handleSubmit} className="login-form">
              ...
              <Form.Item>
                <DatePicker
                  ...
                  placeholder={this.state.activityTime || '请选择活动开始日期'}
                />
              </Form.Item>
              <Form.Item>
                <TimePicker
                  onChange={this.TimePick.bind(this)}
                  placeholder={this.state.activityTime1 || '活动时间'}
                />
              </Form.Item>
              <Form.Item>
                <Input
                  name="activityCredit"
                  type="number"
                  value={this.state.activityCredit}
                  onChange={this.handleChange.bind(this)}
                  prefix={
                    <Icon
                      type="database"
              ...
              <Form.Item>
                <Cascader
                  ...
                  changeOnSelect
                  placeholder={
                    this.state.activityOrganizerName || '请选择活动负责人'
                  }
                />
              </Form.Item>
              <Form.Item>
...
const WrappedModifyActForm = Form.create({ name: 'time_related_controls' })(
  ModifyAct
)
